...
(defun hand-type (hand)
  (declare (optimize debug))
  (let* ((first-chunks (card-chunks hand))
	 (joker-chunks (find #\J first-chunks :key #'car)))
    (if (= (cadar first-chunks) 5) 'five-of-a-kind
	(let ((chunks (remove #\J first-chunks :key #'car)))
	  (when joker-chunks
	    (incf (cadar chunks) (cadr joker-chunks)))
	  (let ((result (cond
			  ...
			  (t 'high-card))))
	    (when joker-chunks
	      (format t "Joker: Hand ~A becomes ~A~%" hand result))
	    result)))))
