...
(defclass part-symbol ()
  ((part-symbol :accessor part-symbol :initarg :part-symbol)
   ...
   (y :accessor y :initarg :y)
   (adjacent-parts :accessor adjacent-parts :initform ())))
(defmethod print-object ((symb part-symbol) stream)
  (print-unreadable-object (symb stream :type t)
    (format stream "~A {~A, ~A} (~{~A~^, ~})"
	    ...
	    (y symb)
	    (x symb)
	    (mapcar #'value (adjacent-parts symb)))))
...
(defun set-adjacent-parts ()
  (let* ((mask (generate-mask))
	...
	(when (and (aref mask y x) (aref parts y x))
	  (dolist (symb (aref mask y x))
	    (pushnew (aref parts y x) (adjacent-parts symb))))))))
...
(defun find-gears ()
  (remove #\*
	  (remove 2 *part-symbols*
		  :test-not #'eql
		  :key (lambda (part) (length (adjacent-parts part))))
	  :test-not #'char=
	  :key #'part-symbol))

(defun gear-ratio (gear)
  (reduce #'* (mapcar #'value (adjacent-parts gear))))

(defun run-part2 (filename)
  (parse-file filename)
  (set-adjacent-parts)
  (let ((gears (find-gears)))
    (reduce #'+ (mapcar #'gear-ratio gears))))
